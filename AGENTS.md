...
- **Refresh semantics**: scans emit `ScanEvent.Discovered` and `ScanEvent.Parsed` in batches up to 100. Only a completed enumeration commits; fatal errors/cancellation discard staging and retain live data. Per-file failures retain old rows. Scan commits are serialized by a central mutex. UI actions enqueue unique per-source `SourceScanWorker` jobs; long scans run as `dataSync` foreground work, survive navigation/backgrounding, and can be rescheduled by WorkManager after process death.
