package com.schwanitz.ui.screens.decadedetail
...
@HiltViewModel
class DecadeDetailViewModel @Inject constructor(
    ...
    fun loadDecade(startYear: Int) {
        val endYear = startYear + 9
        viewModelScope.launch {
            runCatching {
                launch {
                    songRepository.getSongsByDecade(startYear, endYear).collect { _songs.value = it }
                }
                launch {
                    songRepository.getAlbumsByDecade(startYear, endYear).collect { _albums.value = it }
                }
                launch {
                    songRepository.getAlbumArtistsByDecade(startYear, endYear).collect {
                        _artists.value = it
